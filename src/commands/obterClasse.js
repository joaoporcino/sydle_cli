...
const obterClasseCommand = new Command('obterClasse')
    ...
    .action(async (identifier) => {
        try {
            ...
            console.log(`Fetching class ${identifier} and creating folders...`);
            const classId = '000000000000000000000000';
            const classPakageId = '000000000000000000000015';

            // Query filtering by identifier.keyword
            const query = {
                query: {
            ...
            console.log('Phase 1: Collecting class data...');
            ...
            if (allClassesData.length === 0) {
                console.log(`No class found with identifier: ${identifier}`);
                return;
            }

            console.log(`Phase 1 complete. Found ${allClassesData.length} class(es).`);
            ...
            for (const _class of allClassesData) {
                ...
                for (const embField of nestedFields) {
                    try {
                        ...
                        console.error(`Failed to load embedded class for field ${embField.identifier}:`, error instanceof Error ? error.message : String(error));
                    }
                }
                ...
                console.log(`Generated files for class: ${_class.identifier}`);
            }
